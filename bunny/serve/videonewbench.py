...
def eval_model(tokenizer, model, image_processor, context_len,data):
    ...
    answer=data['answer']
    qs =  question 
    qs = MULTI_IMAGE_TOKEN_1 + MULTI_IMAGE_TOKEN_2+ MULTI_IMAGE_TOKEN_3+ MULTI_IMAGE_TOKEN_4+ MULTI_IMAGE_TOKEN_5+ MULTI_IMAGE_TOKEN_6 +MULTI_IMAGE_TOKEN_7 +MULTI_IMAGE_TOKEN_8 + '\n' + qs + '\n' + "Only give the best option."
    ...
    image_tensor = process_videos(image, image_processor, model.config)
...
def check_ans(pred, gt):
    ...
    #     flag = True
    gt_content=gt_content.replace(".","")
    pred=pred.lower().replace(".","")
    # print("real_gt",gt_content,pred)
...
if __name__ == "__main__":
    ...
    resolution = 384
    data_list = {
    "count": ("count.json", "/zhaobai46d/Count_Needle_Order_Ego/video/count", "video", False), # has start & end
    "ego": ("ego.json", "/zhaobai46d/Count_Needle_Order_Ego/video/ego", "video", False), # has start & end
    "needle": ("needle.json", "/zhaobai46d/Count_Needle_Order_Ego/video/needle", "video", False),
    "order": ("order.json", "/zhaobai46d/Count_Needle_Order_Ego/video/order", "video", False)
    }

    data_dir = "/zhaobai46d/Count_Needle_Order_Ego/json"
    ...
    for example in tqdm(dataset):
        ...
        total += 1
        pred=eval_model(tokenizer, model, image_processor, context_len, example)
        gt = example['answer']
    ...
    with open("new_upload_leaderboard.json", "w") as f:
