...
class BunnyMetaForCausalLM(ABC):
    ...
    def encode_images(self, images):
        image_features = self.get_model().get_vision_tower()(images) # per_device_batchsize*frame, 729, 1152
        image_features = self.get_model().mm_projector(image_features) # per_device_batchsize*frame, image_token_number, 3072
    ...
    def prepare_inputs_labels_for_multimodal(self, input_ids, position_ids, attention_mask,
        ...
        if getattr(images_minibatch, 'ndim', 0) == 4:  # batch consists of images, [mini_b, c, h, w]
            if vision_tower is not None:
                image_features_minibatch = self.encode_images(images_minibatch).to(self.device)   # [mini_b, l, c]
            else:
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            cur_input_embeds = self.get_model().embed_tokens(torch.cat(cur_input_ids_noim))  # cur_input_embeds is only text embeddings
            ...
            for i in range(num_images + 1):
                ...
                if i < num_images:
                    try:
                        ...
                        print(f"Error in __getitem__ at index {i}: {e}")
                        print("len(image_features)", len(image_features))
                        print("len(images)",len(images))
                        print("num_images", num_images)
                        print("cur_image_idx", cur_image_idx)
                    cur_image_idx += 1
                    cur_new_input_embeds.append(cur_image_features)  # still is original input order
