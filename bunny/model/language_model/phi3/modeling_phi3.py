...
from .iou_loss import compute_iou
...
class Phi3ForCausalLM(Phi3PreTrainedModel):
    ...
    def is_valid_time_window_list(self, s):
    ...
    def get_time_window(self, s):
        windows = re.findall(r'(\d+)', s)
        try:
            windows_list = []
            for i in range(0, len(windows), 2):
                if int(windows[i]) > int(windows[i+1]):
                    return None
                windows_list.append([int(windows[i]), int(windows[i+1])])
            return windows_list
